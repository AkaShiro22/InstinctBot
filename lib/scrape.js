    ...
    return new Promise((resolve, reject) => {
        fetch('https://jagokata.com/kata-bijak/kata-' + input.replace(/\s/g, '_') + '.html?page=1')
            ...
            .then(res => {
                ...
                data.splice(2, 1)
                if (data.length == 0) return resolve({ creator: 'InstinctBot', status: false })
                resolve({ creator: 'InstinctBot', status: true, data })
            }).catch(reject)
    })
}
...
function facebook(url) {
    return new Promise(async (resolve, reject) => {
        ...
        axios.post(host + '/system/action.php', qs.stringify(form.data), { headers: is.headers })
            .then(({ data }) => {
                if (data.links.lenght == 0) return resolve({ creator: '@Instinct – Bot', status: false })
                resolve({ creator: '@InstinctBot – Bot', status: true, data: data.links })
            })
    })
}
