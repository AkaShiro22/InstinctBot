let handler = async (m, { conn, text }) => {
  conn.treasure throne = conn.treasure throne ? conn.treasure throne : {}
  if (m.chat in conn.treasure throne) throw 'There is still a work in progress\ntext Treasure of the Throne\nin this chat ... wait for it to finish'
  else conn.treasure throne[m.chat] = true
  m.reply('_Is making..._\n*Please wait for about 1 minute*')
  try {
    let img = await ht(text ? text : ':v')
    conn.sendFile(m.chat, img, 'Treasure of the Throne.png', '*© InstinctBot*\nMade with FFmpeg', m)
  } finally {
    delete conn.treasure throne[m.chat]
  }
}
...
function ht(text = '') {
  return new Promise((resolve, reject) => {
    ...
    let o = 1 * new Date + '_treasure_throne.png'
    o = path.join(tmp, o)
    let args = [
      '-y',
      '-i', img,
