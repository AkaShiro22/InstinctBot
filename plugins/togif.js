     ...
     return new Promise((resolve, reject) => {
          ...
          Axios({
               ...
               Axios({
                    ...
                    resolve({
                         status: true,
                         message: "Created By InstinctBot",
                         result: result
                    })
               }).catch(reject)
          }).catch(reject)
...
webp2gifFile('undefined.webp')
  .then(async buffer => {
 conn.sendMessage(m.chat, await require('node-fetch')(buffer.result).then(n => n.buffer()), 'videoMessage', { mimetype: 'video/gif', quoted: m, caption: '```Sticker animeted to gif!```'})
  }) 
