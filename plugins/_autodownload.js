...
handler.all = async function (m, { isPrems, isOwner }) {
    ...
    if (/^.*tiktok/i.test(m.text)) {
        ...
        //     await this.sendVideo(m.chat, json.nowm, '*© INSTINCT-BOT*', m, { thumbnail: buf })
        // }).catch(_ => _)
        let res = await fetch(global.API('hardianto', '/api/download/tiktok', { url: m.text.split(/\n| /i)[0] }, 'apikey'))
        if (!res.ok) throw await `${res.status} ${res.statusText}`
        let json = await res.json()
        await m.reply(global.wait)
        await this.sendVideo(m.chat, json.wm, '© INSTINCT-BOT', m)
    }
    ...
    if (/^.*cocofun/i.test(m.text)) {
        ...
        let json = await res.json()
        await m.reply(global.wait)
        await this.sendVideo(m.chat, json.download, '© INSTINCT-BOT', m)
    }
    ...
    if (/^.*(fb.watch|facebook.com)/i.test(m.text)) {
        facebook(m.text.split(/\n| /i)[0]).then(async res => {
            ...
            await m.reply(global.wait)
            await this.sendVideo(m.chat, isPrems ? json.data[1].url : json.data[0].url, '© INSTINCT-BOT', m)
        }).catch(_ => _)
    }
    ...
    if (/^.*instagram.com\/(p|reel|tv)/i.test(m.text)) {
        igdl(m.text.split(/\n| /i)[0]).then(async res => {
            ...
            for (let { downloadUrl, type } of json) {
                this.sendFile(m.chat, downloadUrl, 'ig' + (type == 'image' ? '.jpg' : '.mp4'), '© INSTINCT-BOT', m, 0, { thumbnail: await (await fetch(downloadUrl)).buffer() })
            }
        }).catch(_ => _)
    }
    ...
    if (/^.*(pinterest.com\/pin|pin.it)/i.test(m.text)) {
        pin(m.text.split(/\n| /i)[0]).then(async res => {
            ...
            await m.reply(global.wait)
            await this.sendVideo(m.chat, json.data.url, '© INSTINCT-BOT', m)
        }).catch(_ => _)
    }
    ...
    if (/^.*twitter.com\//i.test(m.text)) {
        twitter(m.text.split(/\n| /i)[0]).then(async res => {
            ...
            for (let { url } of json.data) {
                this.sendFile(m.chat, url, 'ig' + (/mp4/i.test(url) ? '.mp4' : '.jpg'), '© INSTINCT-BOT', m)
            }
        }).catch(_ => _)
    }
...
*Server y2mate:* ${usedServer}
          `.trim(),
            await (await fetch(thumb)).buffer(), '© INSTINCT-BOT', 'AUDIO', `.yta ${vid.url}`, 'VIDEO', `.yt ${vid.url}`)
    }
